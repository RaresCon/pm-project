...
static button rev_button = { REV_BUTTON, 0, false };
...
void use_buzzer()
{
    ...
    if (current_data->external.dist < 15) {
        digitalWrite(BUZZER_PIN, HIGH);
    } else if (current_data->external.dist < 50) {
        digitalWrite(BUZZER_PIN, LOW);
        delay(100);
        digitalWrite(BUZZER_PIN, HIGH);
        delay(100);
    } else if (current_data->external.dist < 70) {
        digitalWrite(BUZZER_PIN, LOW);
        delay(200);
        digitalWrite(BUZZER_PIN, HIGH);
        delay(200);
    } else if (current_data->external.dist < 150) {
        digitalWrite(BUZZER_PIN, LOW);
        delay(400);
        digitalWrite(BUZZER_PIN, HIGH);
        delay(400);
